...
Window               WINDOW('Search for...'),AT(,,320,180),FONT('Segoe UI',10),RESIZE,GRAY,HLP('SearchFor.htm'), |
  ...
  HIDE
                       BUTTON('...'),AT(301,5,14,10),USE(?cmdLookupSearchStringFilename),HIDE,TIP('Lookup Sear' & |
  'ch String Filename')
  ...
  ,?bIncludeBinary),TIP('Includes files with non-printable characters.'),TRN
  ...
  ,?bFileListFromFile),TIP('Get the list of files to search from the specified file'),TRN
                       PROMPT('F&ile'),AT(5,102),USE(?szFileListFilename:Prompt),HIDE,TRN
                       ENTRY(@S255),AT(35,102,262,10),USE(SearchOptions.szFileListFilename,,?szFileListFilename), |
  HIDE
                       BUTTON('...'),AT(301,102,14,10),USE(?cmdLookupFileListFilename),HIDE,TIP('Lookup File L' & |
  'ist Filename')
                       PROMPT('&Paths'),AT(5,102),USE(?szSearchPath:Prompt),TRN
                       COMBO(@S255),AT(35,102,245,10),USE(SearchOptions.szSearchPath,,?szSearchPath),VSCROLL,ALRT(MouseRight), |
  ...
  TIP('Search Path Starting Folder')
                       ...
                       CHECK(' Search S&ubdirectories'),AT(35,117),USE(SearchOptions.bSearchSubdirectories,,?bSearchSubdirectories), |
  TIP('Searches for matching files in the current directory and all subdirectories.'),TRN
                       PROMPT('Le&vels:'),AT(139,117),USE(?nLevels:Prompt),TRN
  ...
  'tory levels to search.  If this value is blank then all subdirectories will be searched.')
                       PROMPT('&Files'),AT(5,132),USE(?szFileMask:Prompt),TRN
                       COMBO(@s255),AT(35,132,280,10),USE(SearchOptions.szFileMask,,?szFileMask),VSCROLL,ALRT(MouseRight), |
  ...
  'rtical bars (|) ,  semicolons (;), or spaces.')
                       PROMPT('E&xclude'),AT(5,147),USE(?szExcludeMask:Prompt),TRN
                       COMBO(@s255),AT(35,147,280,10),USE(SearchOptions.szExcludeMask,,?szExcludeMask),VSCROLL,ALRT(MouseRight), |
  ...
  'ay be delimited by vertical bars (|) ,  semicolons (;), or spaces.')
                       BUTTON('&Load Saved Results'),AT(35,162,70,14),USE(?cmdLoadSavedResults)
                       BUTTON('&Search'),AT(140,162,70,14),USE(?cmdSearch),DEFAULT
                       BUTTON('Search Ne&w Tab'),AT(245,162,70,14),USE(?cmdSearchNew)
                     END
...
Resizer.Init PROCEDURE(BYTE AppStrategy=AppStrategy:Resize,BYTE SetWindowMinSize=False,BYTE SetWindowMaxSize=False)
  ...
  SELF.SetStrategy(?bIncludeBinary, Resize:FixLeft+Resize:FixTop, Resize:LockSize) ! Override strategy for ?bIncludeBinary
