...
Window               WINDOW('Options'),AT(,,530,320),FONT('Segoe UI',10),DOUBLE,TILED,GRAY,HLP('UserOptions.htm'), |
  ...
  'ted property file.')
                       END
                       CHECK(' &Link Paths and Files with Find Text'),AT(357,137),USE(glo:SyncPathWithPattern),TIP('Recall Pat' & |
  'hs and Files when you select prior Find text from the drop list.')
                       ...
                       CHECK(' Auto Size Result &Columns'),AT(357,157),USE(glo:AutoSizeResultColumns)
  ...
  'o disable Restore Point Feature')
                       ...
                       GROUP,AT(282,220,234,68),USE(?CopyrightBox),BOXED,TRN
                         STRING('Kwik Source Search'),AT(336,226,127),USE(?ProductName),FONT(,,,FONT:bold),CENTER,TRN
                         STRING(@s22),AT(336,234,127),USE(loc:szVersion),CENTER,TRN
                         STRING('© Copyright  2011-2019'),AT(336,242,127),USE(?Copyright),CENTER,TRN
  ...
  CENTER,TRN
                         STRING('Registered to'),AT(336,269,127),USE(?strRegisteredTo),CENTER,TRN
                         STRING(@S28),AT(336,278,127),USE(RegisteredTo),CENTER,TRN
                       END
                       BUTTON('Register Now'),AT(369,292,,12),USE(?cmdRegisterNow),HIDE
