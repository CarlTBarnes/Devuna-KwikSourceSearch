...
bMatchPatternStartOfLine BOOL                              ! 
bMatchPatternEndOfLine BOOL                                ! 
bUseRegularExpressions BOOL(TRUE)                          ! 
bSearchSubdirectories BOOL                                 ! 
...
szFileMask           LIKE(szFileMask)                      ! 
                     END                                   ! 
Window               WINDOW('Kwik Source Search'),AT(,,600,273),FONT('Segoe UI',10,,,CHARSET:ANSI),RESIZE,ALRT(03DCh), |
  ALRT(CtrlEnd),ALRT(CtrlHome),ALRT(DownKey),ALRT(PgDnKey),ALRT(PgUpKey),ALRT(UpKey),COLOR(, |
  COLOR:Yellow),ICON('kss.ico'),GRAY,IMM,MAX,HLP('Main.htm'),STATUS(-1,50,200,70),SYSTEM,TIMER(5)
                       TOOLBAR,AT(0,0,600,18),USE(?Toolbar1),COLOR(00F2E4D7h)
  ...
  'elete<0DH,0AH>in Results List...<09H>[Ctrl+Delete]')
                         BUTTON,AT(382,2,18,14),USE(?cmdUndoDelete),ICON('UndoDelete.ico'),FLAT,TIP('Undo Delete' & |
  '<0DH,0AH>Ctrl+Z')
                         PANEL,AT(403,3,1,12),USE(?Separator2),BEVEL(1)
  ...
  '.<09H>[Ctrl+R]')
                         ...
                         BUTTON,AT(446,2,18,14),USE(?cmdEdit),ICON('SendTo.ico'),FLAT,TIP('Send To Command<09H>[Ctrl+E]')
                         PANEL,AT(466,3,1,12),USE(?Separator4),BEVEL(1)
                         BUTTON,AT(469,2,18,14),USE(?cmdUserOptions),ICON('UserOptions.ico'),FLAT,TIP('Options ...')
                         BUTTON,AT(489,2,18,14),USE(?cmdLayout),ICON('splith.ico'),FLAT,TIP('Switch to Horizontal Layout')
                         BUTTON,AT(509,2,18,14),USE(?cmdHelp),ICON('help.ico'),FLAT,TIP('Help')
                         STRING(@S255),AT(539,4,,8),USE(szTitle),RIGHT
                         BUTTON,AT(571,4,10,8),USE(?cmdSaveWarn),ICON('save-warn.png'),DISABLE,FLAT,SKIP
                       END
                       TEXT,AT(462,1,136,254),USE(?sciControl:Region),FONT(,,COLOR:BTNTEXT)
                       ...
                       LIST,AT(1,28,456,226),USE(?ResultList),VSCROLL,ALRT(CtrlC),ALRT(CtrlE),ALRT(CtrlF),ALRT(CtrlF4), |
  ...
  '(2)|M~Location~S(1024)@s255@175L(2)|M~Text~S(1024)@s255@'),FROM(EasyListPrintQueue)
                       REGION,AT(460,0,2,260),USE(?SplitterBar),FILL(COLOR:SteelBlue),IMM
...
Resizer.Init PROCEDURE(BYTE AppStrategy=AppStrategy:Resize,BYTE SetWindowMinSize=False,BYTE SetWindowMaxSize=False)
  ...
  SELF.RemoveControl(?CurrentSearch)                       ! Remove ?CurrentSearch from the resizer, it will not be moved or sized
  SELF.RemoveControl(?ResultList)                          ! Remove ?ResultList from the resizer, it will not be moved or sized
